...
const DeviceProvider = ({children}: ReactChildren) => {
	...
	const [ledgerApp, setLedgerApp] = useState<typeof FilecoinApp>(null);
	const [currentAccount, setCurrentAccount] = useState<string | null>(null);
	const [indexAccount, setIndexAccount] = useState<number>(0);
	const [accounts, setAccounts] = useState<string[] | null>(null);
	const getAccounts = async (ledgerApp: typeof FilecoinApp) => {
		const paths = [];
		for (
			let i = 0;
			i < Number(process.env.VITE_NUMBER_OF_WALLET_ACCOUNTS);
			i += 1
		) {
			paths.push(`m/44'/${process.env.VITE_LOTUS_NODE_CODE}'/0'/0/${i}`);
		}
