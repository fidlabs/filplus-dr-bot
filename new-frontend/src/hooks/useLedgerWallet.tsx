...
const numberOfWalletAccounts = process.env.VITE_NUMBER_OF_WALLET_ACCOUNTS || '0';
const lotusNodeCode = process.env.VITE_LOTUS_NODE_CODE;
	...
	const sign = async (
		...
		const signedMessage = handleErrors(
			await ledgerApp.sign(
				`m/44'/${process.env.VITE_LOTUS_NODE_CODE}'/0'/0/${indexAccount ?? "0"}`,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const verifyAPI = createVerifyAPI(sign as any, getAccounts);
