...
export class VerifyAPI {
	...
	async listVerifiers() {
		...
		const listOfVerifiers = await this.methods.buildArrayData(verifiers, (a) =>
			this.load(a),
		);
		const returnList = [];
	...
	async proposeVerifier(
		...
		const tx = this.methods.rootkey.propose(
			this.methods.verifreg.addVerifier(verifierAccount, datacap),
		);
	...
	encodeRemoveDataCapParameters(message: {
		...
		const orderedProposalParams = [
			message.verifiedClient,
			message.dataCapAmount,
			message.removalProposalID,
		];
	...
	async proposeRemoveVerifier(
		...
		// Not address but account in the form "t01004", for instance
		const tx = this.methods.rootkey.propose(
			this.methods.verifreg.removeVerifier(verifierAccount),
		);
		const res = await this.methods.sendTx(
			this.client,
			indexAccount,
	...
	async approveVerifier(
		...
		const tx = this.methods.rootkey.approve(parseInt(transactionId, 10), {
			...add,
			from: fromAccount,
		});
		const res = await this.methods.sendTx(
	...
	async cancelVerifier(
		...
		// Not address but account in the form "t01003", for instance
		const add = this.methods.verifreg.addVerifier(verifierAccount, datacap);
		const tx = this.methods.rootkey.cancel(parseInt(transactionId, 10), {
			...add,
			from: fromAccount,
		});
		const res = await this.methods.sendTx(
	...
	async listVerifiedClients() {
		...
		const listOfVerified = await this.methods.buildArrayData(verified, (a) =>
			this.load(a),
		);
		const returnList = [];
	...
	async cancelPending(msig, tx, from, wallet) {
		...
		return await this.send(
			m1_actor.cancel(parseInt(tx.id), tx.tx),
			from,
			wallet,
		);
	}
	...
	async newMultisig(signers, threshold, cap, from, wallet) {
		const tx = this.methods.init.exec(
			this.methods.multisigCID,
			this.methods.encode(this.methods.msig_constructor, [
				signers,
				threshold,
				cap,
				1000,
		...
		const [addr] = this.methods.decode(
			['list', 'address'],
			decode(Buffer.from(receipt.Return, 'base64')),
		);
		return addr;
	}
